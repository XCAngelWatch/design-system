...
This file provides guidance to OpenAI Codex / 通用 coding agent when working with code in this repository.
...
- 品牌 token 与布局姿态证据:`brand-spec.md`(其 `--bg/--accent` 等是**证据 token**,运行时实现一律用 `project/styles/tokens.css` 的 `--aw-*`)
...
# 2. 全部 page fragments 加载 + 注册校验(无浏览器,Node 静默测试)
node -e "
global.window = {};
const fs = require('fs');
...
- 应用消费入口(`<ConfigProvider>` + locale + renderEmpty)→ `packages/web/src/app/AntdConfig.tsx`(参考 `pages/config-provider.js` 可复制示例)
- 业务页面消费 → `packages/web`(antd v6 原生 + `src/components/` 业务封装,不用 Pro Components)

> 职责切分:`antd.ts` 只管 `theme.token`,`components.ts` 只管 `theme.components`,`AntdConfig.tsx` 是消费入口。不要把 `theme.components` 写进 `antd.ts`。
