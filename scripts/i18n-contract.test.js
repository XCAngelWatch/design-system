...
assert.match(router, /delete loadedLocaleScripts\[routeId\]/);
assert.doesNotMatch(router, /loadedLocaleScripts\[routeId\] = false/);
assert.match(router, /aria-labelledby="' \+ groupLabelId/);
assert.match(router, /aria-current="page"/);
assert.match(router, /slot\.setAttribute\('aria-busy', 'true'\)/);
assert.match(router, /slot\.setAttribute\('aria-busy', 'false'\)/);
assert.match(router, /focusHeading/);
assert.match(router, /heading\.focus\(\{ preventScroll: true \}\)/);
assert.match(router, /event && event\.type === 'hashchange'/);
assert.match(systemCss, /@media \(prefers-reduced-motion: reduce\)[\s\S]*animation-iteration-count:\s*1\s*!important/);
assert.match(systemCss, /--aw-doc-sidebar-width:\s*240px/);
assert.match(systemCss, /html\[lang\]:not\(\[lang\^="zh"\]\)[\s\S]*--aw-doc-sidebar-width:\s*272px/);
assert.match(systemCss, /grid-template-columns:\s*var\(--aw-doc-sidebar-width\)\s+minmax\(0,\s*1fr\)/);
assert.match(systemCss, /html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.nav a\s*\{[^}]*white-space:\s*normal/);
assert.match(systemCss, /@media \(max-width:\s*1100px\)\s*\{[\s\S]*?\.app\s*\{[^}]*display:\s*block/);
assert.match(systemCss, /@media \(max-width:\s*1100px\)\s*\{[\s\S]*?\.app-side\s*\{[^}]*position:\s*fixed[^}]*height:\s*100dvh[^}]*transform:\s*translateX\(-102%\)/);
assert.match(systemCss, /\.doc-nav-open\s+\.app-side\s*\{[^}]*transform:\s*translateX\(0\)[^}]*visibility:\s*visible/);
assert.match(systemCss, /\.doc-nav-open\s+\.doc-nav-backdrop\s*\{[^}]*display:\s*block/);
assert.match(router, /data-doc-nav-toggle aria-controls="app-side" aria-expanded="false"/);
assert.match(router, /sidebar\.setAttribute\('inert', ''\)/);
assert.match(router, /event\.key === 'Escape'[\s\S]*?syncNavigationDrawer\(false, \{ restoreFocus: true \}\)/);
assert.match(extrasCss, /\.locale-toggle-fixed/);
assert.match(extrasCss, /\.toolbar-controls/);
assert.match(extrasCss, /html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.btn\s*\{[^}]*min-height:\s*32px[^}]*height:\s*auto[^}]*white-space:\s*normal/);
assert.match(extrasCss, /html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.form-row\s*>\s*\.lbl\s*\{[^}]*min-width:\s*132px[^}]*white-space:\s*normal/);
assert.match(extrasCss, /html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.tabs\s*\{[^}]*overflow-x:\s*auto/);
assert.match(extrasCss, /html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.tabs a\s*\{[^}]*white-space:\s*nowrap/);
assert.match(extrasCss, /html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.tag,\s*html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.status-dot\s*\{[^}]*white-space:\s*nowrap/);
assert.match(extrasCss, /html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.card-title,[\s\S]*?html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.surface h4\s*\{[^}]*overflow-wrap:\s*break-word[^}]*word-break:\s*normal/);
assert.match(extrasCss, /html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.actions,[\s\S]*?html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.ph-actions\s*\{[^}]*flex-wrap:\s*wrap/);
assert.match(extrasCss, /html\[lang\]:not\(\[lang\^="zh"\]\)\s+table th\s*\{[^}]*white-space:\s*normal[^}]*line-height:\s*1\.35/);
assert.match(extrasCss, /html\[lang\]:not\(\[lang\^="zh"\]\)\s+table \.mono\s*\{[^}]*white-space:\s*nowrap/);
assert.match(extrasCss, /html\[lang\]:not\(\[lang\^="zh"\]\)\s+\.i18n-table-scroll\s*\{[^}]*overflow-x:\s*auto[^}]*max-width:\s*100%/);
