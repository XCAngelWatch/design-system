...
| Figma 文案清单 | `docs/decisions/audits/2026-07-08-figma-language-inventory.md` | 模块词汇、字段名、状态名、排除样例 |
...
本轮进一步校验了 Figma tree 的 canvas 结构，并分析了外部参考目录 `/Users/david/Documents/workspaces/xctech/tms25/code/design-system-angelwatch`。

新增结论：

1. `design-system-angelwatch` 是旧站抽取出的品牌包、token 包和应用 kit，可用于补充颜色、控件尺寸、侧栏 / 顶栏布局、表格与表单密度。
2. 当前仓库仍是实现权威；外部参考不得覆盖静态 SPA、AntD v6、`--aw-*` token、无运行时 CDN 和 `file://` 可用约束。
3. 旧 Vue 2 / Element UI 代码、营销生成物、被排除的旧 Figma 页面不迁入当前项目。
4. 已新增 `AI_DESIGN_SYSTEM.md` 作为 AI 可直接读取的 Markdown 总入口。
5. 已新增 `project/index.html#/ai-reference` 作为网页端 AI 开发入口，并在“规范”分组注册。
6. 外部参考详情见 `docs/decisions/audits/2026-07-09-angelwatch-external-design-system-audit.md`。
