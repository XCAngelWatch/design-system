...
本文件给未来 AI 编程使用。它汇总从 Figma 与既有实现整理的业务观察，以及 Design System 的视觉和交互规则。业务观察是带来源的设计快照，不是后端/OpenAPI 或产品决策的替代品。

## AI 读取入口

优先读取：

1. `AI_DESIGN_SYSTEM.md`：最短路径入口，说明分域权威边界、页面范式、外部参考边界和提交前校验。
2. `project/index.html#/ai-reference`：网页端 AI 开发入口，适合从浏览器直接跳到列表、统计、账户权限、地图、推送和 i18n 范式。
3. `docs/evidence/angelwatch-business-capabilities.json`：带来源的机器可读设计快照；记录能力观察与设计映射，但不是业务契约。
4. `docs/evidence/figma-frame-manifest.json`：Figma canvas 映射与排除结论。
5. `brand-spec.md`：品牌 token、字体、布局姿态和证据摘要。
6. `docs/decisions/audits/2026-07-11-figma-legacy-gap-audit.md`：Figma / 旧系统证据治理和 TMS 1.0 隔离决定。
...
| 问题域 | 权威来源 | 本文件的角色 |
| --- | --- | --- |
| 视觉 token、组件交互、无障碍、布局 | 当前 Design System 规范与 `contracts/` | 给出可复用设计规则 |
| 业务字段、状态、权限、API | 后端实现、OpenAPI 与后端领域契约 | 保存待核验观察，不裁决业务冲突 |
| 信息架构、页面范围、用户流程 | 已批准产品需求与 Figma / OpenDesign | 映射页面蓝图，不把 mock 提升为产品决定 |
| 消费者实现事实 | `tms2.5-web-ui` 当前源码与测试 | 记录真实落点，不虚构尚不存在的包、目录或生成链 |

来源缺失或冲突时保持待核验，禁止让本文件、`project/pages/` 和 evidence manifest 互相引用完成自证。Figma 中标记为废案、临时方案、占位图层或测试数据的内容不得进入最终规范；旧 Vue 2 / Element UI 只能作为历史参考。
...
- 如果外部品牌包与现行来源冲突，视觉 token、组件交互、无障碍和布局以当前 Design System 为准；字段、状态、权限与 API 以后端/OpenAPI 为准；信息架构与流程以已批准产品需求/Figma 为准；消费者实现事实以当前应用仓源码与测试为准。
...
以下导航词汇来自已记录的 Figma 产品观察。是否属于当前产品、如何分组以及流程如何衔接，仍由已批准产品需求/Figma 决定：
...
| 批量 / 不可逆操作 | `App.useApp().modal.confirm` | 明示数量、对象和后果，默认焦点在取消；禁止静态 Modal API |
...
- 当前唯一已验证消费者是 sibling 单应用仓库 `tms2.5-web-ui`；实现事实以其 `src/App.tsx`、`src/styles/tokens.css`、`src/theme/antd.ts`、`src/theme/components.ts` 与测试为准。
- 根节点结构固定为 `ConfigProvider > App > application`；Message、Notification、Modal 从 `App.useApp()` 获取，禁止静态 API 绕过 locale、theme 和 context。
- Light / Dark 使用显式 `--aw-*` 与 ConfigProvider theme token 协同切换；当前应用从活动 CSS 变量构建 AntD theme，不存在可假定使用的 `tokens.json` 生成链或 tenant theme。未来扩展必须先更新 Design System 决策与消费者契约。
- `scrollToFirstError` 属于具体 `Form`；自建表单控件必须透传 `value/onChange/id/ref/disabled/status`。
- `AppTable` 默认使用 AntD Table；固定行高的大数据场景优先 `virtual` 且同时提供数值型 `scroll.x/scroll.y`，是否更换内核只由性能测试决定。
- 包装 AntD v6 组件时透传 `classNames/styles` Semantic DOM API，禁止依赖内部 DOM 层级选择器。
- 普通详情页使用面包屑；仅沉浸式子流程使用返回按钮，不同时放置等价的两套返回导航。
- 数量、金额、百分比和文件大小列右对齐并使用 `tabular-nums`；状态列与操作列禁止换行。

## 跨模块流程设计快照（实现前核验）

以下阶段与规则用于表达已观察到的页面交互。流程范围以已批准产品需求/Figma 为准，字段、状态、权限与 API 以后端/OpenAPI 为准。

| 流程 | 已观察阶段 | 设计参考 | 页面参考 |
| --- | --- | --- | --- |
| 配置资产 | 上传解析 → 字段校验 → 目标范围 → 影响确认 → 提交追踪 | 解析字段只读；前端不解析 APK 签名或证书；终态进入 Result | `advanced-form`, `upload`, `avatar-result` |
| 批量分配 | 筛选 → 勾选/导入 → 冲突检查 → 跳过/覆盖 → 结果 | 冲突展示来源；覆盖需要独立权限；结果区分失败、冲突和跳过 | `tree-list`, `cascader`, `user-mgmt-page` |
| 远程会话 | 等待设备 → 连接中 → 进行中 → 结束/失败 → 日志 | 会话状态独立于推送批次；关闭页面后仍可追踪；禁止旧 iframe 和隐藏凭据 | `device-center-page`, `progress`, `feedback` |
| 告警处置 | 告警 → 定位 → 修复 → 记录 → 复核 | 操作日志与处置记录互链但不合并；保留处理人、时间、动作和结果 | `map-page`, `device-center-page`, `ops-page` |

旧 MDM、Resource Package、Watch System、Focus、Kiosk 和 Launcher 只在 evidence 快照中保留 `pending/exclude` 设计结论，不是当前产品导航或核心蓝图。是否启用必须由已批准产品需求/Figma 决定，并由后端/OpenAPI 确认相应业务能力。

## 模块业务观察（非业务契约）

本节字段、状态、动作、权限 key 和枚举均来自历史设计观察。实现前必须逐项核对现行后端/OpenAPI；页面范围与操作流必须核对已批准产品需求/Figma。下文的交互提示只在业务事实确认后生效。

### 应用市场

当前设计快照记录：应用管理、广告管理、应用分类、版本管理、包名、版本号、安装包、下载数、审核、上架、下架、安装明细、所属机构、操作人、更新时间、版本总数、下载总数、禁止卸载、禁止清除数据、签名方式、适配机型、可见范围、自动安装、自动更新、应用排序。
...
当前设计快照记录：升级文件、版本号、基线版本、适配机型、安卓版本、大小、是否强制、可见范围、更新推送、配置文件、推送固件包。
...
当前设计快照记录：批次号、推送时间、推送方式、消息类型、指定设备编号、推送目标、设备推送结果统计、推送设备详情、成功、失败、总数。
...
当前设计快照记录：设备管理、设备详情、设备分组、型号管理、参数管理、参数模板、联机方案、设备型号、系统版本、固件版本、电池电量、IP地址、SN、IMEI-IMSI-icc_id、位置上报、欠费设备、基带版本、内核版本、编译信息、KSN、到期时间、客户名、资源包版本、MDM配置版本、主/子SDK版本、经纬度、wifi SSID、蓝牙地址、备注、上线时间、锁屏状态、信息上报间隔、位置自动上报。
...
当前设计快照记录：获取设备日志、锁定解锁设备、数据流量控制、重置设备密码、恢复出厂设置、推送文件到设备、APN设置、WIFI设置、应用黑白名单、推送记录、推送设备详情、operationType 词汇表、扫码授权、壁纸、热点、电源模式、GPS开关、电源键开关、logcat配置、清除应用数据、消息通知、查找设备、调试开关、蓝牙开关、修改型号、设备名称。
...
当前设计快照记录：设备查询、定位查询、轨迹查询、围栏管理、围栏警告、开始时间、结束时间、起点、终点、围栏类型、事件类型、关联设备数、告警计数、门店管理、经度、纬度、围栏半径。
...
当前设计快照记录：数据统计、设备活跃数据、活跃设备、活跃设备数、设备激活数据、激活设备、设备激活数、固件统计、统计详情、升级文件统计详情、近7日、当日活跃、七天内活跃、30天内活跃、当日激活、七天内激活、30天内激活。
...
当前设计快照记录：机构管理、运营商管理、角色管理、账号管理、配置权限、OSS节点、操作日志、字典管理、菜单管理。
...
以下规范化用于保持设计示例一致；最终产品术语仍须经产品/Figma 确认，字段值和枚举值仍须经后端/OpenAPI 确认。
...
```text
按 AngelWatch TMS 设计系统实现 [模块名] 页面。先用后端/OpenAPI 核验字段、状态、权限与 API，用已批准产品需求/Figma 核验信息架构、页面范围、流程与术语；docs/ai-coding-design-reference.md、evidence manifest 和 project/pages 只作为带来源的设计快照。视觉 token、组件交互、无障碍和布局遵循当前 Design System，消费者实现事实以 tms2.5-web-ui 当前源码与测试为准。不得复制废案、临时方案或占位图层，不硬编码业务颜色，不使用 Pro Components，并通过现有校验。
```

实现组件时可使用：

```text
按 AngelWatch TMS 组件规范实现 [组件名]。API 需支持 className/style/classNames/styles/data-*/aria-* 透传，受控/非受控模式清晰，文案走 i18n，主题走 --aw-* token 和 ConfigProvider。反馈 API 从 App.useApp() 获取；表单控件透传 id/ref。状态必须用颜色 + 文案 + 形状，不只靠颜色。破坏性操作默认焦点不得落在确认按钮。
```
