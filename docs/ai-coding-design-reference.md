...
- 应用列表真实列：应用名(logo) / 机构 / 包名 / **版本总数 versionCount** / **下载总数 downloadCount**(旧系统列表无分类/状态/更新时间列,真实区分字段是版本总数+下载总数)。支持表格/卡片视图切换、列设置、数据范围切换(本级及子级)。
...
- 术语决策:已上架 = **Published**(非 Figma 的 Released);已下架 = **Unpublished**(非 Removed)。
...
- 设备列表真实列:selection → SN(logo+sn,fixed,点击进详情) → 备注 → 在线状态 → 机构 → 型号 → 系统版本 → 上线时间(可排序) → 锁屏状态 → IMEI-IMSI-icc_id → 操作。列表无电量/心跳(这些在详情页)。支持表格/卡片视图、列设置、数据范围切换(本级及子级)。
...
- 远程控制快捷指令 16 项(在线才开放,离线禁用):软件信息 / 流量控制 / 设备日志 / 应用安装 / 应用卸载 / 设备密码 / 设备锁定 / 全部刷新 / 恢复出厂 / WIFI设置 / 调试开关 / 清警告 / 蓝牙开关 / 修改型号 / 消息通知 / 查找设备 / logcat。每项复用推送引擎生成推送任务。
...
- 围栏列表真实列:名称 / **shape 围栏类型(circle 圆形 / polygon 多边形)** / **eventType(enter 进 / leave 出 / enter_leave 进出)** / 机构 / 备注 / **deviceCount 关联设备数** / 操作(设置权限/修改/删除)。顶部有**告警数 badge**跳转告警日志。
...
- 术语决策:围栏 = **Geofence / Geocerca**(非 Figma 的 Fence)。
...
- 应用管理状态：Draft、**Published**(已上架,非 Released)、**Unpublished**(已下架,非 Removed)。采纳生产措辞,见 `copywriting.js` 术语锁定表。
...
- 机构管理字段:机构名称、上级机构、登录账号。⚠ "机构编码 / 所属运营商 / 设备数量"为前瞻字段(旧系统未落地,来自 Figma 或运营商管理),应标注。自身机构禁用所有操作;新增机构后弹密码对话框展示自动生成密码+一键复制。
