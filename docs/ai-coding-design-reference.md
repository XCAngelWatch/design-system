...
3. `docs/evidence/angelwatch-business-capabilities.json`：机器可读业务契约和当前有效能力边界。
...
- Light/Dark/tenant 必须同时切换 `--aw-*` 与 ConfigProvider theme token；两者由同一 `tokens.json` 生成，不使用 CSS 选择器假定覆盖 AntD CSS-in-JS。
...
旧 MDM、Resource Package、Watch System、Focus、Kiosk 和 Launcher 只在证据 manifest 中保留 `pending/exclude` 结论，不是当前产品导航或核心蓝图。需要启用时，必须先由当前有效 Figma 或现行业务契约重新确认。
...
必须优先使用：设备管理、设备详情、设备分组、型号管理、参数管理、参数模板、联机方案、设备型号、系统版本、固件版本、电池电量、IP地址、SN、IMEI-IMSI-icc_id、位置上报、欠费设备、基带版本、内核版本、编译信息、KSN、到期时间、客户名、资源包版本、MDM配置版本、主/子SDK版本、经纬度、wifi SSID、蓝牙地址、备注、上线时间、锁屏状态、信息上报间隔、位置自动上报。
