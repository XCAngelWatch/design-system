...
<section class="section" id="toast">
  <p class="section-eyebrow"><span data-i18n="toast:text.001">通用组件 · 瞬时反馈</span></p>
  <h2><span data-i18n="toast:text.002">提示 Toast / Tooltip / Popover</span></h2>
  <p class="lede"><span data-i18n="toast:text.003">Toast 浮于右上角，自动消失 (3s)，用于操作结果。Tooltip 用于解释行内文字。Popover 承载结构化补充内容。</span></p>
...
<div class="toast-stack">
  ...
  <div class="toast error"><div class="ico">×</div><div class="body"><strong><span data-i18n="toast:text.010">推送失败</span></strong><span data-i18n="toast:text.011">34 台设备未确认接收</span></div><div class="close">×</div></div>
</div>
    </div>
    <div class="surface">
...
<div style="padding:60px 0 0;position:relative;height:200px">
  ...
  <div class="subsection">
    ...
    <p class="lede" style="margin-bottom:12px"><span data-i18n="toast:text.023">Toast / Message / Notification 在 antd v6 中各有职责。本节统一规约 TMS 内 4 类瞬时反馈的位置、栈策略与自动消失时长。</span></p>
    ...
    <div class="alert info" style="margin-top:12px"><div class="ico">i</div><div class="content"><strong><span data-i18n="toast:text.050">位置规则：</span></strong><span data-i18n="toast:text.051">同一会话内不同时使用 topRight + bottomRight。新 Toast 入场动画 200ms 滑入；超过栈上限时最旧者立即消失（不动画），新 Toast 滑入。</span></div></div>
  </div>

  <div class="subsection">
    <h3><span data-i18n="toast:text.052">边界对照 · Toast / Alert / Modal / Result</span></h3>
    ...
    <div class="surface" style="padding:0;overflow:hidden">
      <table class="tech-table">
        ...
        <tbody>
          <tr><td><b>Toast / Message</b></td><td><span data-i18n="toast:text.059">否</span></td><td><span data-i18n="toast:text.060">否</span></td><td><span data-i18n="toast:text.061">否（浮层）</span></td><td><span data-i18n="toast:text.062">非阻塞、轻量结果反馈、可忽略</span></td></tr>
    ...
    <div class="alert warning" style="margin-top:12px"><div class="ico">!</div><div class="content"><strong><span data-i18n="toast:text.076">不要混用：</span></strong><span data-i18n="toast:text.077">错误用 Toast 而不是 Alert → 用户错过即丢失；失败用 Modal 而不是 Toast → 频繁打断流程；批量结果用 Toast 而不是 Result → 失败明细列不下。</span></div></div>
  </div>
  ...
  <div class="subsection">
    ...
    <div class="rich-toast-grid">
          ...
          <div class="toast info" style="position:relative;width:100%">
            ...
            <div class="body">
              ...
              <div class="rt-acts">
                ...
                <button class="btn btn-link btn-sm" style="padding:0;color:var(--aw-text-3)"><span data-i18n="toast:text.084">取消推送</span></button>
              </div>
            </div>
            <div class="close">×</div>
          </div>
      ...
      <div class="rt-card">
        <div class="rt-cap"><span data-i18n="toast:text.088">③  折叠展开 · 错误明细</span></div>
        <div class="rt-body">
          <div class="toast error expanded" style="position:relative;width:100%">
            ...
            <div class="body">
              ...
              <small><span data-i18n="toast:text.090">详情见下方明细</span></small>
              <div class="rt-detail">
  ...
  <div class="subsection">
    ...
    <div class="alert error"><div class="ico">×</div><div class="content"><strong><span data-i18n="toast:text.098">不要用 Toast 显示需要用户做选择的内容：</span></strong><span data-i18n="toast:text.099">Toast 自动消失，用户来不及反应。需要决策走 Modal。</span></div></div>
    <div class="alert error" style="margin-top:8px"><div class="ico">×</div><div class="content"><strong><span data-i18n="toast:text.100">不要让 4 个 Toast 堆叠遮挡 topbar：</span></strong><span data-i18n="toast:text.101">同方向超过 3 个时挤掉最旧的；超过 5 个考虑改 Notification Center 入口（topbar 铃铛图标）。</span></div></div>
    <div class="alert error" style="margin-top:8px"><div class="ico">×</div><div class="content"><strong><span data-i18n="toast:text.102">不要用错误 Toast 而不展示 traceId：</span></strong><span data-i18n="toast:text.103">用户截图给 SRE 时缺少定位信息。错误必须含 traceId 或错误码。</span></div></div>
  </div>
  ...
  <div class="subsection">
    ...
    <div class="surface" style="font-family:var(--aw-font-mono);font-size:12px;line-height:1.7;color:var(--aw-text-2)">
      ...
      <div style="margin-top:4px"><span data-i18n="toast:text.106">--aw-shadow-2 (Toast) · --aw-radius (容器) · --aw-dur-base (滑入)</span></div>
      <div style="margin-top:4px"><span data-i18n="toast:text.107">栈上限：Toast 3 / Notification 富 2 / Message 3</span></div>
    </div>
  </div>
</section>
</div>
